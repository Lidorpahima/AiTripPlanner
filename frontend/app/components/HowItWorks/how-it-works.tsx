    ...
    <section className=" py-16 md:py-24 verflow-hidden">
      <div className="mx-auto max-w-7xl px-4 sm:px-6">
        ...
        >
          {steps.map((step, index) => (
            <motion.div 
              key={index} 
              variants={fadeIn}
              className="flex flex-col items-center p-6 rounded-xl transform transition-all duration-300 hover:scale-105 hover:bg-grey-800 shadow-lg bg-gray-50"
            >
              <div className="mb-4 p-3 bg-blue-50 rounded-full">
                {step.icon}
              </div>
              <h3 className="text-xl text-center font-semibold mb-2 text-gray-800 hover:underline">
                {step.title}
              </h3>
              <p className="text-center text-gray-600">
                {step.description}
              </p>
              <div className="mt-4 flex items-center justify-center w-8 h-8 rounded-full bg-blue-100 text-blue-500 font-semibold">
                {index + 1}
              </div>
            </motion.div>
          ))}
        ...
        >
          <Link 
            href="/fastplan"
            className="inline-block bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-medium py-3 px-8 rounded-lg shadow-lg transition-all duration-300"
          >
            Start Planning Now
          </Link>
        </motion.div>
